...
export const CharacterBuildingView = ({
  ...
  const [actionUnits, setActionUnits] = useState<
    { id: string; action: string; strategies: string[] }[]
  >(characterBuilding.actionUnits);
    ...
    >
      <Grid>
        <Typography fontSize={39} fontWeight="light" color="primary.main">
          {characterBuilding?.character?.name} en{" "}
          {characterBuilding?.scene?.description}
        </Typography>
      ...
      <Grid container spacing={2} sx={{ mt: 2 }}>
        <FormControl fullWidth>
          ...
          <Select
            ...
            variant="filled"
            value={characterBuilding.center}
          >
        ...
        <TextField
          ...
          variant="filled"
          fullWidth
          placeholder="Es la cena de cumpleaños del abuelo"
          label="Circunstancias de la escena"
          value={characterBuilding.sceneCircumstances}
        />
        <TextField
          type="text"
          variant="filled"
          fullWidth
          placeholder="El Doctor acaba de decirle a Brick que el abuelo sí tiene cáncer"
          label="Circunstancias previas"
          value={characterBuilding.previousCircumstances}
        />
        <TextField
          type="text"
          variant="filled"
          fullWidth
          placeholder="Brick está medio borracho y solo quiere evadirse"
            ...
            >
              ...
              <Grid size={{ xs: 4 }}>
                <FormControl fullWidth>
                  ...
                  <Select
                    value={relation.character.name}
                    onChange={(e) =>
                      handleRelationChange(index, "character", e.target.value)
                    }
                  >
            ...
            >
              <Grid size={{ xs: 1 }}>
                <Button
                  variant="outlined"
                  color="secondary"
                  onClick={() => handleRemoveActionUnit(unitIndex)}
                >
                  <Delete color="error" />
                </Button>
              </Grid>
              <Grid size={{ xs: 3 }}>
                <TextField
                  ...
                  value={unit.action}
                  onChange={(e) =>
                    handleActionUnitChange(unitIndex, e.target.value)
                  }
                />
              </Grid>
              <Grid size={{ xs: 6 }}>
                <Grid container spacing={2} alignItems="baseline">
                  ...
                  <Button
                    variant="contained"
                    color="primary"
                    onClick={() => handleAddStrategy(unitIndex)}
                    sx={{ mt: 1 }}
                  >
                    <Add />
                  </Button>
